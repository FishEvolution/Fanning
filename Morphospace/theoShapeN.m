classdef theoShapeN
   ...
   %% Meshes
        ...
        % create plate mesh
        function model = PlateMesh3D(obj,thickness,area,nEle)
            tmodel = createpde();
            m2 = generateMesh2D(obj.harm, nEle);
            ele = zeros(m2.nEle,3);
            for i = 1:m2.nEle
                ele(i,:) = m2.ele(i).nodes; %* sqrt(area);
            end

            node = vertcat(m2.x',m2.y') * sqrt(area);

            nFace = m2.nEle;

            g = geometryFromMesh(tmodel,node, ele',ones(nFace,1));
